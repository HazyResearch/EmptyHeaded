...
object PlanUtil {
  ...
  private def createAttrWithSelectionInfo(attr:Attr,
    ...
    (attr,
      attrToSelection.get(attr).map(selection => {
        if (selection.isEmpty) {
          ""
        } else {
          selection(0).operation.value
        }
      }),
      attrToSelection.get(attr).map(selection => {
        if (selection.isEmpty) {
  ...
  def attrNameAgnosticRelationEquals(output1:Rel,
                                     ...
                                     joinAggregates:Map[String, Aggregation]): Option[Map[Attr, Attr]] = {
    ...
    val zippedAttrs = (
      rel1.attrs.values.map(attr => createAttrWithSelectionInfo(attr, attrToSelection)),
      rel1.attrs.values.map(attr => attrMap.get(attr)),
      rel2.attrs.values.map(attr => createAttrWithSelectionInfo(attr, attrToSelection))).zipped.toList
    if (zippedAttrs
