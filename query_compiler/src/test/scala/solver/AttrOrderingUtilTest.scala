...
class AttrOrderingUtilTest extends FunSuite {
  test("Can reorder attributes so that equality selected ones are at the front") {
    val partitioned = AttrOrderingUtil.partition_equality_selected(
      List("a","b","x","c", "y", "z"),
      List(
        Selection("y", EQUALS(), SelectionLiteral("1")),
        Selection("x", EQUALS(), SelectionLiteral("1")),
        Selection("z", EQUALS(), SelectionLiteral("100")),
        Selection("z", EQUALS(), SelectionLiteral("5"))
      ))
  ...
  test("Can order attributes, taking into account GHD structure and what's equality-selected") {
    ...
    val child1 = new GHDNode(List(
      OptimizerRelFactory.createOptimizerRel("a", "x")), Array(Selection("x", EQUALS(), SelectionLiteral("0"))))
    val child2 = new GHDNode(List(
      OptimizerRelFactory.createOptimizerRel("b","y")), Array(Selection("y", EQUALS(), SelectionLiteral("0"))))
    val child3 = new GHDNode(List(
      OptimizerRelFactory.createOptimizerRel("c","z")),  Array(Selection("z", EQUALS(), SelectionLiteral("0"))))
    ...
    val ordering = AttrOrderingUtil.getAttributeOrdering(
      ...
      List(Selection("x", EQUALS(), SelectionLiteral("0")),Selection("y", EQUALS(), SelectionLiteral("0")), Selection("z", EQUALS(), SelectionLiteral("0"))))
