...
class QueryCompilerTest extends FunSuite {
  ...
  test("lollipop query") {
    val optimized = QueryCompiler.optimize("Lollipop(a,b,c) :- Edge(a,b),Edge(b,c),Edge(a,c),Edge(a,d).").head
    println(optimized)
  }

  test("barbell query") {
    val optimized = QueryCompiler.optimize("Barbell(a,b,c) :- Edge(a,b),Edge(b,c),Edge(a,c),Edge(a,d),Edge(d,e),Edge(e,f),Edge(d,f).").head
    println(optimized)
  }
}
